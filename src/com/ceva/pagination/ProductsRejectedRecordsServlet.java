...
public class ProductsRejectedRecordsServlet extends HttpServlet {
	...
	Logger logger = Logger.getLogger(ProductsUnAuthRecordsServlet.class);
	...
	public JSONObject getPersonDetails(int totalRecords, HttpServletRequest request) throws SQLException, ClassNotFoundException {
		...
		String sql ="select PRODUCT_ID,PRODUCT_NAME,PRODUCT_PRICE,CATEGORY_ID,SUB_CATEGORY_ID,STATUS,MAKER_ID,MAKER_DTTM from"
				...
				+ "  (select PRODUCT_ID,PRODUCT_NAME,PRODUCT_PRICE,CATEGORY_ID,SUB_CATEGORY_ID,  decode(STATUS,'M','Un-Authorized','A','Approved','R','Rejected',STATUS) STATUS,"
				+ "MAKER_ID,to_char(MAKER_DTTM,'DD-MM-YYYY HH24:MI:SS') MAKER_DTTM  from ONLINE_PRODUCTS_MASTER  where STATUS='R' order by MAKER_DTTM)) "
				+ "where rn between "+
	...
	public int getTotalRecordCount() throws SQLException {
		...
		String sql ="select count(*) count from  "
				...
				+ " decode(STATUS,'M','Un-Auth','A','Auth',STATUS) STATUS,MAKER_ID,to_char(MAKER_DTTM,'DD-MM-YYYY HH24:MI:SS') MAKER_DTTM  "
				+ "from ONLINE_PRODUCTS_MASTER  where STATUS='R' order by MAKER_DTTM))";
		try {
