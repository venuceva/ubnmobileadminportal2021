...
import com.ceva.util.ArrayUtil;
import com.ceva.util.Conversion;
import com.ceva.util.CryptoException;
...
public class CryptoUtil {
	...
	public static byte[] udkEMV200(byte[] MK, byte[] IV, int ATC, int B, int H)
	...
	private static byte[] encryptEMV2000(byte[] X, byte[] Y, int j)
			throws CryptoException {
		// int h=8; //tree height
	...
	/*
	 * public static byte[] decrypt(byte[] X,byte[] Z,int j)throws
	 * CryptoException{ // int h=8; //tree height int b=4; //no of children
	 * 
	 * byte[] XL=new byte[8]; byte[] XR=new byte[8]; ArrayUtil.split(X,XL,XR);
	 * byte[] ZL=new byte[8]; byte[] ZR=new byte[8]; ArrayUtil.split(Z,ZL,ZR);
	 * 
	 * String mod=Integer.toHexString(j%b); byte[]
	 * bMod=Conversion.hex2byte(mod); byte[]
	 * result=CryptoUtil.des3ECBINV(ArrayUtil.concat(XL,XR,XL),ZL);
	 * 
	 * byte[] YL=ArrayUtil.xor(result,bMod); byte[]
	 * re1=CryptoUtil.des3ECBINV(ArrayUtil.concat(XL,XR,XL),ZR); byte[]
	 * re2=ArrayUtil.xor(re1,bMod); byte[] p={(byte)0xF0}; byte[]
	 * YR=ArrayUtil.xor(re2,p); return ArrayUtil.concat(YL,YR); }
	 */
