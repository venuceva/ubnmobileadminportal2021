...
function buildtable(jsonArray,divid)
{
	...
	$.each(jsonArray, function(index,jsonObject){
			...
			if(divid == "LMT_TBody")
				...
				htmlString = htmlString + "&nbsp&nbsp<a class='btn btn-success' href='#' id='limitview' index="+i+" val="+i+" title='View' data-rel='tooltip' onclick='viewfun("+i+")'> <i class='icon icon-book icon-white'></i></a>";
...
$(document).on('click','a',function(event) {
	...
	{
		$('#feeCode').val(code);
		$("#form1")[0].action='<%=request.getContextPath()%>/<%=appName %>/limitfeesettingsfeedataactive.action';
		$("#form1").submit();
		return true; 
