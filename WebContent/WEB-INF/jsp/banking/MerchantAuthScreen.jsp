...
$(function() { 
	...
	if(json.length == 0) {
		...
		if( header.indexOf("#") != -1 ) {
			...
			//headerData = "<tr><th><input type=\"checkbox\" name=\"select-all\" id=\"select-all\" /></th>";
		...
		$ .each(json, function(i, v) {
			...
			appendTxt = "<tr class="+addclass+" id='"+rowindex+"' index='"+rowindex+"'>" 
					+ "<td ><input class='center-chk' type='checkbox' value='" + v.USER_ID + "' /></td>"
					+ "<td>" + v.USER_ID + "</td>";
					
			if(status == 'MERC') {
				appendTxt+="<td>"+ v.merchantName + "</td>" 
						 +"<td>"+ v.MERCH_ADMIN + "</td>" ;
			} else if(status == 'STORE') {
				appendTxt+="<td>"+ v.storeName + "</td>" 
						+"<td>"+ v.merchantID + "</td>" 
						+"<td>"+ v.merchantName + "</td>";
 			} else {
				appendTxt+="<td>"+ v.storeId + "</td>" 
						 +"<td>"+ v.merchantID + "</td>"
						 +"<td>"+ v.serialNo + "</td>" ;
			}  		 
	...
	$("#btn-submit").click(function(event) {
		...
		if(searchIDs.length == 0) {
			...
			$("#error_dlno").text("");
			var url="${pageContext.request.contextPath}/<%=appName %>/merchantAuthorizationCnf.action"; 
			$("#form1")[0].action=url;
			$("#form1").submit(); 
					...
					<div class="box-content" id="primaryDetails"> 
						<fieldset>  
							<table width="950" border="0" cellpadding="5" cellspacing="1"
								...
								<tr class="even">
									<td colspan="4"> No records to authorize, please press back to Merchant Management.</td>
								</tr>
							</table>
						</fieldset>
					</div>
