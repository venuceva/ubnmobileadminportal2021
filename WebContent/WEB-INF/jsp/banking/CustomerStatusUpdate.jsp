...
$(document).ready(function(){
	...
	$.each(jsonLinks, function(index, v) {
		...
		if(v.name=="wcustomeServicesAct"){
			
			$("#WALLET").prop("disabled", false);
			
			
		}
		if(v.name=="mcustomeServicesAct"){
			$('#apptype').val("MOBILE");
	...
	myApp = myApp || (function () {
	    var pleaseWaitDiv = $('<div class="modal hide" id="pleaseWaitDialog" data-backdrop="static" data-keyboard="false"><div class="modal-header">Processing...</div><div class="modal-body"><div class="progress progress-info progress-striped active"><div class="bar"  style="width: 100%;"></div></div></div></div>');
	    return {
			...
			$.getJSON("postJson.action", queryString9,function(data) {
				myApp.hidePleaseWait();
					...
					if(mb=="MOBILE" && v_telco!="Txn Pin is Pending"){
						...
						$("#appname").text("Mobile Banking");
					/* 	<i class='icon icon-share icon-white'></i> */
						$("#actiontype").html("<a class='btn btn-warning' href='#' id='btn-submitwalletm' index='1' val='1' title='Customer Services' data-rel='tooltip' >View</a>");
						$("#mt").css({"display":""});	
					}else{
						$("#mt").css({"display":"none"});
					}
					...
					if(wl=="WALLET" && v_telco!="Txn Pin is Pending"){
							...
							$("#actiontypew").html("<a class='btn btn-warning' href='#' id='btn-submitwalletmw' index='2' val='2' title='Customer Services' data-rel='tooltip' > <i class='icon icon-share icon-white'></i></a>");
						$("#wt").css({"display":""});
					}else{
						$("#wt").css({"display":"none"});
						}
	...
	$('#actiontype').click(function(){ 
			...
			if($("#form1").valid()) { 
				myApp.showPleaseWait();
 				$("#form1")[0].action='<%=request.getContextPath()%>/<%=appName %>/fetchCustomerService.action'; 
				$('#form1').submit();
				 return true; 
			} else { 
				return false;
	...
	$('#actiontypew').click(function(){ 
		//$("#form1").validate(acctRules);
		$("#apptype").val("WALLET");
		$("#customercode").val($("#customercodew").val());
		if($("#form1").valid()) { 
			myApp.showPleaseWait();
				$("#form1")[0].action='<%=request.getContextPath()%>/<%=appName %>/fetchwalletServiceAct.action'; 
			$('#form1').submit();
			 return true; 
		} else { 
			return false;
					...
					<div class="box-content" id="primaryDetails">
						...
						<%-- <div class="section-inner">
									<div class="segmented-control" style="width: 600px; color: #00ADEF;font-weight: bold;font-size: 14px;">
					                    	<input type="radio" name="ecust" id="MOBILE"  onclick="funval(this.id)">
					                    	<input type="radio" name="ecust" id="WALLET" onclick="funval(this.id)">
					                     	<label for="MOBILE" data-value="Mobile Customer Services" ><img src='${pageContext.request.contextPath}/images/Phone.png' align='left' style='width: 40px; height: 40px;'><span style="color: #000000;font-weight: bold;font-size: 12px;">Mobile Customer Services</span></label>
                    						<label for="WALLET" data-value="Wallet Customer Services"  ><img src='${pageContext.request.contextPath}/images/Wallet.png' align='left' style='width: 40px; height: 40px;'><span style="color: #000000;font-weight: bold;font-size: 12px;">Wallet Customer Services</span></label>
					                  </div>
									</div> --%>
							<table width="950" border="0" cellpadding="5" cellspacing="1"
								class="table table-striped table-bordered bootstrap-datatable " >
								<%-- <tr class="odd" >
									<td width="20%"><label><strong>Application Type <font color="red">*</font></strong></label></td>
									<td width="50%" colspan="4">
									
									
									<strong>Mobile Banking</strong>&nbsp;&nbsp;&nbsp;&nbsp;<input type="radio" id="MOBILE" name="ecust" checked onclick="funval(this.id)" />&nbsp;&nbsp;&nbsp;&nbsp;
									<strong>Wallet</strong>&nbsp;&nbsp;&nbsp;&nbsp;<input type="radio" id="WALLET" name="ecust" onclick="funval(this.id)" />
									</td>	
								<!-- <td></td>	
	 							<td></td> -->
								</tr> --%>
						...
						<table class="table table-striped table-bordered bootstrap-datatable "  style="width: 100%;" >
						  ...
						  <tbody > 
							  <tr id="mt" style="display:none" >
						  			...
						  			<td><span id="appname" name="appname" class='label label-success'  align='center' ></span></td>
							   ...
							   <tr id="wt" style="display:none" >
						  			...
						  			<td><span id="appnamew" name="appnamew" class='label label-info'  align='center'></span></td>
