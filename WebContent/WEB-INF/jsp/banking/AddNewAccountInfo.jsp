			...
			$.getJSON("postJson.action", queryString9,function(data) {
				myApp.hidePleaseWait();
					...
					if (v_message =='SUCCESS')
					{
						...
						$("#actiontype").html("<a class='btn btn-warning' href='#' id='btn-submitwalletm' index='1' val='1' title='Modify Customer' data-rel='tooltip' > View</a>");
						$("#mt").css({"display":""});	
					}else{
						$("#mt").css({"display":"none"});
					}
					...
					if(wl=="WALLET"  && v_telco!="Txn Pin is Pending"){
							...
							$("#actiontypew").html("<a class='btn btn-warning' href='#' id='btn-submitwalletmw' index='2' val='2' title='Modify Customer' data-rel='tooltip' >View</a>");
						$("#wt").css({"display":""});
					}else{
						$("#wt").css({"display":"none"});
						}
...
<%-- 	$('#btn-submit').click(function(){ 
	...
	$('#actiontype').click(function(){ 
		...
		if($("#form1").valid()) { 
			myApp.showPleaseWait();
				$("#form1")[0].action='<%=request.getContextPath()%>/<%=appName %>/addnewaccountDet.action'; 
			$('#form1').submit();
			 return true; 
		} else { 
			return false;
						...
						<div class="section-inner">
									<div class="segmented-control" style="width: 600px; color: #00ADEF;font-weight: bold;font-size: 14px;">
					                    	<input type="radio" name="ecust" id="MOBILE"  onclick="funval(this.id)">
					                     	...
					                     	<label for="MOBILE" data-value="Mobile Customer Add Account" ><img src='${pageContext.request.contextPath}/images/Phone.png' align='left' style='width: 40px; height: 40px;'><span style="color: #000000;font-weight: bold;font-size: 12px;">Mobile Customer Add Account</span></label>
