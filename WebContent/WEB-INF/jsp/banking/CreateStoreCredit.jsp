...
var storeCreditRules = {
   rules : {
    storeCreditAmt : { required : true }
   },  
   messages : {
    storeCreditAmt : { 
       required : "Please enter Store Credit Amount."
        }
   } 
  };
...
function createStoreLimit(){
	...
	if($("#form1").valid()){	
		$("#form1")[0].action='<%=request.getContextPath()%>/<%=appName %>/getStoreCreditCreateConfirmScreenAct.action';
		$("#form1").submit();
		return true;
	}else{
		return false;
						...
						<div class="box-header well" data-original-title>
							...
							<div id="limitDetails" class="box-content">
								<fieldset>  
										...
										class="table" >
											<tr class="even">
												<td><strong><label for="Store Limit">Store Credit Amount<font color="red">*</font></label></strong></td>
												<td><input name="storeCreditAmt" type="text" id="storeCreditAmt" class="field" value="${responseJSON.storeCreditAmt}" required='true' ></td>
												<td ></td>
							                    <td></td>
											</tr> 
									</table>
