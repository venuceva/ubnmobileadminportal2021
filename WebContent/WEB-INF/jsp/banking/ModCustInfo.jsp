...
$( "#customerId" ).keyup(function() {
	...
	$.each(jsonLinks, function(index, v) {
		...
		if(v.name=="wmodregcustinfo"){
			
			$("#WALLET").prop("disabled", false);
			
			
		}
		if(v.name=="mmodregcustinfo"){
			$('#apptype').val("MOBILE");
			...
			$.getJSON("postJson.action", queryString9,function(data) {
				myApp.hidePleaseWait();
					...
					if (v_message =='SUCCESS')
					{
						...
						/*  if(v_telco=="Txn Pin is Pending"){
							$("#stpin").css({"display":""});
							$("#customercodem").val(custid); 
						}else{
							$("#stpin").css({"display":"none"});
						}  */
...
$('#actiontypew').click(function(){ 
	...
	if($("#form1").valid()) { 
		myApp.showPleaseWait();
			$("#form1")[0].action='<%=request.getContextPath()%>/<%=appName %>/modregwalletcustinfoDet.action'; 
		$('#form1').submit();
		 return true; 
	} else { 
		return false;
...
$('#btn-txnpin').click(function(){ 
	//$("#form1").validate(acctRules);
		$("#apptype").val("MOBILE");
		$("#customercode").val($("#customercodem").val());
	
		$("#form1")[0].action='<%=request.getContextPath()%>/<%=appName %>/modregcustinfoDetPin.action'; 
		$('#form1').submit();
		...
		<div id="content" class="span10">
		    <div>
				<ul class="breadcrumb">
				  ...
				  <li><a href="#">Modify Registered Account </a></li>
				</ul>
			</div>
		<table height="3">
			 <tr>
						...
						<div class="section-inner">
									<div class="segmented-control" style="width: 600px; color: #00ADEF;font-weight: bold;font-size: 14px;">
					                    	...
					                    	<input type="radio" name="ecust" id="WALLET" onclick="funval(this.id)">
					                     	<label for="MOBILE" data-value="Mobile Customer Modify" ><img src='${pageContext.request.contextPath}/images/Phone.png' align='left' style='width: 40px; height: 40px;'><span style="color: #000000;font-weight: bold;font-size: 12px;">Mobile Customer Modify</span></label>
                    						<label for="WALLET" data-value="Wallet Customer Modify"  ><img src='${pageContext.request.contextPath}/images/Wallet.png' align='left' style='width: 40px; height: 40px;'><span style="color: #000000;font-weight: bold;font-size: 12px;">Wallet Customer Modify</span></label>
					                  </div>
									</div>
            ...
            <div class="progress progress-striped active">
                <div class="bar" style="width: 100%;"></div>
					...
					<div class="box-content" id="ex-mytable"  style="display:none"> 
						...
						<table class="table table-striped table-bordered bootstrap-datatable "  style="width: 100%;">
						<tr><td width="50%">
						<span id="statusmessage" style="color:#0000ff;font-size: 15px;"></span>
						</td><td width="50%"> 
						<span id="stpin" style="display:none"><a  class="btn btn-danger" href="#" id="btn-txnpin" name="btn-txnpin">Set Txn Pin</a> </span>
						</td></tr>
						</table>
					</div> 
		</div> 
