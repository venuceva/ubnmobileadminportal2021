						...
						<table class="table table-striped table-bordered bootstrap-datatable dataTable" 
						  ...
						  <tbody id="tbody_data1"> 
						  		...
						  		<s:bean name="com.ceva.base.common.bean.JsonDataToObject" var="jsonToList">
										<s:param name="jsonData" value="%{accountPropBean.multiData}"/>  
								...
								<s:iterator value="#jsonToList.data" var="mulData1"  status="mulDataStatus" > 
										...
										<td><s:property value="#mulDataStatus.index+1" /></td>
											<s:set var="statusclass"  scope="session"/> 
											<s:generator val="%{#mulData1}"
												var="bankDat" separator="," >  
												<s:iterator status="itrStatus">  
							                	
							                	<s:if test="#itrStatus.count <= 3">   
							                	<td id='<s:property />'><s:property /></td> 
							                	</s:if>
							                	<s:if test="#itrStatus.count == 4">   
							                	
							                	<%--  <s:set value="%{#mulData1}" var="statusclass"/>  --%>
							                	 
							                	 <s:generator val="%{#mulData1}" count="4" separator="," var="myAtt" />
												<%
...
%>
							            ...
							            <s:if test="#statusclass == 'E'" >

							            <input type="checkbox"  class='center-chk' name="<s:property value="#mulDataStatus.index" />" checked="checked" id="<s:property  value="#mulDataStatus.index" />"/>
							            <s:else >
							            <input type="checkbox"  class='center-chk' name="<s:property value="#mulDataStatus.index" />"  id="<s:property  value="#mulDataStatus.index" />"/>
							            </s:else>
							            </s:if></td>
									</tr> 
								</s:iterator>
