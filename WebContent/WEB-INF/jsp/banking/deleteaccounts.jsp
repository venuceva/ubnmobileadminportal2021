...
var queryString1 = "method=primaryaccountdetails&refno="+vid+"&custcode="+vval+"&makerid="+ $('#makerid').val()+"&apptype="+$('#apptype').val();
				...
				myApp.hidePleaseWait();
					...
					if (v_message =='SUCCESS')
					{
						...
						if(mb=="MOBILE" && v_telco!="Txn Pin is Pending"){
							...
							$("#actiontype").html("<a class='btn btn-warning' href='#' id='btn-submitwalletm' index='1' val='1' title='Unregister Account' data-rel='tooltip' >View</i></a>");
							$("#mt").css({"display":""});	
						}else{
							$("#mt").css({"display":"none"});
						}
						...
						if(wl=="WALLET" && v_telco!="Txn Pin is Pending"){
								...
								$("#actiontypew").html("<a class='btn btn-warning' href='#' id='btn-submitwalletmw' index='2' val='2' title='Unregister Account' data-rel='tooltip' > <i class='icon icon-trash icon-white'></i></a>");
							$("#wt").css({"display":""});
						}else{
							$("#wt").css({"display":"none"});
							}
		...
		$('#actiontype').click(function(){ 
			...
			if($("#form1").valid()) { 
				myApp.showPleaseWait();
					$("#form1")[0].action='<%=request.getContextPath()%>/<%=appName %>/delAccountsDet.action'; 
				$('#form1').submit();
				 return true; 
			} else { 
				return false;
						...
						<div class="section-inner">
									<div class="segmented-control" style="width: 600px; color: #00ADEF;font-weight: bold;font-size: 14px;">
					                    	<input type="radio" name="ecust" id="MOBILE"  onclick="funval(this.id)">
					                     	<label for="MOBILE" data-value="Mobile Delete Account" ><img src='${pageContext.request.contextPath}/images/Phone.png' align='left' style='width: 40px; height: 40px;'><span style="color: #000000;font-weight: bold;font-size: 12px;">Mobile Delete Account</span></label>
					                  </div>
									</div>
