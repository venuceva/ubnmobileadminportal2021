...
$(document).ready(function(){
	...
	$.each(jsonLinks, function(index, v) {
		...
		if(v.name=="wsearchcustomeract"){
			
			$("#WALLET").prop("disabled", false);
			
			
		}
		if(v.name=="msearchcustomeract"){
			$('#apptype').val("MOBILE");
				...
				myApp.hidePleaseWait();
					...
					if (v_message =='SUCCESS')
					{
						...
						if(mb=="MOBILE" ){ /* && v_telco!="Txn Pin is Pending" */
							...
							$("#actiontype").html("<a class='btn btn-warning' href='#' id='btn-submitwalletm' index='1' val='1' title='Customer View' data-rel='tooltip' >View</a>");
							$("#mt").css({"display":""});	
						}else{
							$("#mt").css({"display":"none"});
						}
						...
						if(wl=="WALLET" && v_telco!="Txn Pin is Pending"){
								...
								$("#actiontypew").html("<a class='btn btn-warning' href='#' id='btn-submitwalletmw' index='2' val='2' title='Customer View' data-rel='tooltip' > <i class='icon icon-book icon-white'></i></a>");
							$("#wt").css({"display":""});
						}else{
							$("#wt").css({"display":"none"});
							}
	...
	$('#actiontype').click(function(){ 
		...
		if($("#form1").valid()) { 
			myApp.showPleaseWait();
				$("#form1")[0].action='<%=request.getContextPath()%>/<%=appName %>/searchcustomerdet.action'; 
			$('#form1').submit();
			 return true; 
		} else { 
			return false;
...
$('#actiontypew').click(function(){ 
	...
	if($("#form1").valid()) { 
		myApp.showPleaseWait();
			$("#form1")[0].action='<%=request.getContextPath()%>/<%=appName %>/searchwalletcustomerdet.action'; 
		$('#form1').submit();
		 return true; 
	} else { 
		return false;
						...
						<div class="section-inner">
									<div class="segmented-control" style="width: 600px; color: #00ADEF;font-weight: bold;font-size: 14px;">
					                    	...
					                    	<input type="radio" name="ecust" id="WALLET" onclick="funval(this.id)">
					                     	<label for="MOBILE" data-value="Mobile Customer Search" ><img src='${pageContext.request.contextPath}/images/Phone.png' align='left' style='width: 40px; height: 40px;'><span style="color: #000000;font-weight: bold;font-size: 12px;">Mobile Customer Search</span></label>
                    						<label for="WALLET" data-value="Wallet Customer Search"  ><img src='${pageContext.request.contextPath}/images/Wallet.png' align='left' style='width: 40px; height: 40px;'><span style="color: #000000;font-weight: bold;font-size: 12px;">Wallet Customer Search</span></label>
					                  </div>
									</div>
