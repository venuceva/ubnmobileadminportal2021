...
var cntFun = '${responseJSON2.VER_CNT}';
console.log("cntFun"+cntFun);

var ideFun = '${responseJSON2.IDE_CNT}';
console.log("ideFun"+ideFun);

window.setTimeout( refreshGrid, 150000);
var oTable;
function refreshGrid() { 
	...
	var formInput='';
	json = "";
 	$.getJSON('<%=ctxstr%>/<%=appName %>/livedata.action',formInput,function(data) {  
...
function fillJqGrid(data1) {  
	...
	//$("#user-tbody").empty();
		...
		var cntFun = '${responseJSON2.VER_CNT}';
   			...
   			"<td><a id='identified' role='identified'>"+ideFun+"</a></td>"+
   			"<td><a id='verified' role='verified'>"+cntFun+"</a></td>"+
   			 "</tr>"; 
   			
   			
   		$("#user-tbody1").append(appTxt);
	...
	$.each(json, function(index, v) {
		...
		var appendTxt = "<tr class="+addclass+" id='"+rowindex+"' index='"+rowindex+"'> "+
			...
			"<td>"+undefinedcheck(v.AMOUNT)+" </td>"+
 			"<td>"+undefinedcheck(v.ACCOUNT)+" </td>"+
			"<td>"+undefinedcheck(v.PY_SHRTCODE)+" </td>"+  
 			"<td><a id='opener' role='"+undefinedcheck(v.RRN)+"'>"+undefinedcheck(v.RRN)+"</a></td>"+
...
<script>
  ...
  $(document).on('click',"a",function(event) {
	 ...
	 $("#form1")[0].action='<%=request.getContextPath()%>/<%=appName %>/fraudact.action'; 
	 $('#form1').submit();
	 }
	 else if(id == 'verified')
	 {
		 ...
		 $('#form1').submit();
		  	...
		  	var formInput='RRN='+rrn_id;
			   	...
			   	$.getJSON("<%=ctxstr%>/<%=appName %>/drillTxnDetails.action",formInput,function(data) {  
			   		console.log(data);
...
<!-- 		<div class="row-fluid sortable">  
		...
		<div class="box span12" id="groupInfo">
			...
			<div >  
					...
					class="table table-striped table-bordered" id="DataTables_Table">
						<thead>
							<tr>
								<th>Identified</th>
								<th>Verified</th>
