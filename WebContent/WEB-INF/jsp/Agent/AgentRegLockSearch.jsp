			...
			$('#application').on('change', function (e) {
				...
				$('#fname').val("");
			        ...
			        $('#btn-submit').live('click', function () { 
			        	...
			        	if(funcheck()==1){
			        		var queryString = "entity=${loginEntity}&method=searchAgent&link=LOCK&fname="+$('#fname').val()+"&mnumber="+$('#application').val();
			        		
			        			$.getJSON("postJson.action", queryString,function(data) { 
			        				if(data.finalCount==0){
			        					$('#errormsg').text("No records available in "+$('#application').val());
			        				}else{ 
					    				var url="${pageContext.request.contextPath}/<%=appName %>/agentreglocksearch.action"; 
					    				$("#form1")[0].action=url;
					    				$("#form1").submit();
					    				return true;
			        				 }
