			...
			$(function () { 
				...
				$( "#fname" ).keyup(function() {
			        ...
			        $('#btn-submit').live('click', function () { 
			        	...
			        	if(funcheck()==1){
			        		if($('#application').val()=="DSA"){
			        			
			        			var queryString = "entity=${loginEntity}&method=searchAgent&link=UPDATE&fname="+$('#fname').val()+"&mnumber="+$('#application').val();
				        		
			        			$.getJSON("postJson.action", queryString,function(data) { 
			        				if(data.finalCount==0){
			        					$('#errormsg').text("No records available");
			        				}else{ 
			        					var queryString1 = "entity=${loginEntity}&method=searchAgent&link=AUTHPEND&fname="+$('#fname').val()+"&mnumber="+$('#application').val();
						        		
					        			$.getJSON("postJson.action", queryString1,function(data) { 
					        				
					        				if(data.finalCount==0){
					        					
					        						var url="${pageContext.request.contextPath}/<%=appName %>/dsalimitupdatesearch.action"; 
								    				$("#form1")[0].action=url;
								    				$("#form1").submit();
								    				return true;
						...
						if($('#application').val()=="AGENT"){
							...
							var queryString = "entity=${loginEntity}&method=AgentDetails&link=PRODUCT&fname="+$('#fname').val()+"&mnumber="+$('#application').val();
		        			...
		        			$.getJSON("postJson.action", queryString,function(data) { 
		        				if(data.finalCount==0){
				        			...
				        			$.getJSON("postJson.action", queryString1,function(data) { 
				        				...
				        				if(data.finalCount==0){
						    				var url="${pageContext.request.contextPath}/<%=appName %>/agentproductupdatesearch.action"; 
						    				$("#form1")[0].action=url;
						    				$("#form1").submit();
						    				return true;
				        				}else{
	...
	<div id="content" class="span10">
			...
			<div>
				<ul class="breadcrumb">
					...
					<li><a href="dsalimitupdate.action">Product Update</a></li>
