...
function buildtable(jsonArray,divid)
{
	...
	$.each(jsonArray, function(index,jsonObject){
			...
			htmlString = htmlString + "<td id=ACTION_TYPE"+ i + " name=ACTION_TYPE >" + jsonObject.ACTION_TYPE + "</td>";
 			htmlString = htmlString + "<td id=sno name=sno ><a id=sno class='btn btn-info' href='#' onclick=fun('" + jsonObject.P_SEQNO + "') title='Agent Request Approval' data-rel='tooltip'><i class='icon icon-book icon-white'></i></a></td>";
...
function fun(v){
	...
	$("#refno").val(v);
	if($("#application").val()=="AGNPAYBILLAIRTIME_PENDING" || $("#application").val()=="WALPAYBILLAGN_PENDING" || $("#application").val()=="WALAGNOTBANK_PENDING" || $("#application").val()=="AGNCRDCSHWTDOTH_PENDING" || $("#application").val()=="AGNCRDCSHWTDOTH_PENDING" || $("#application").val()=="AGNCRDCSHWTDOTH_PENDING" || $("#application").val()=="AGNCRDCSHWTDOTH_PENDING"){
		
		$("#form1")[0].action='<%=request.getContextPath()%>/<%=appName %>/agentrequestapprovalinfo.action';
		$("#form1").submit();
		return true;
	}
	if($("#application").val()=="AGNPAYBILLAIRTIME_COMPLETED" || $("#application").val()=="WALPAYBILLAGN_COMPLETED" || $("#application").val()=="WALAGNOTBANK_COMPLETED" || $("#application").val()=="AGNCRDCSHWTDOTH_COMPLETED" || $("#application").val()=="AGNCRDCSHWTDOTH_COMPLETED" || $("#application").val()=="AGNCRDCSHWTDOTH_COMPLETED" || $("#application").val()=="AGNCRDCSHWTDOTH_COMPLETED"){
