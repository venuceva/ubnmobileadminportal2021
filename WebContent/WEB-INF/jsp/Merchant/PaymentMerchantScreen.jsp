...
$(document).ready(function () { 
	...
	$.each(json, function(i, v) {
		...
		"<td >"+colindex+"</td>"+
		"<td>"+v.ORGANIGATION_ID+"</span></td>"+	
		"<td>"+v.ORGANIGATION_NAME+"</span> </td>"+ 
		 "<td>"+v.MERCHANT_TYPE+"</span> </td>"+
		"<td>"+v.MAKER_ID+"</span></td>"+
		"<td>"+v.MAKER_DT+"</span></td>"+
		"<td style='display:none'>"+v.QR_CODE+"</span></td>"+
		"<td><a id='merchant-pay-code' class='btn btn-success' href='#' index="+colindex+" title='Payment Code View' data-rel='tooltip'><i class='icon icon-plus icon-white'></i></a></td></tr>"; 
...
$(document).on('click','a',function(event) {
	...
	if(v_id == 'SEARCH_NO') {
		var billerId = $(this).attr('value').split("@")[1];
    	var queryString = 'billerCode='+billerCode;
						...
						$(searchTdRow).each(function(indexTd) {  
							if (indexTd == 1) {
								...
								// email ids
								 ...
								 qrcode=$(this).text().trim();
							 }  
						}); 

						queryString += 'orgid='+billerCode+'&orgname='+accountNumner+'&qrcode='+qrcode;
					
						if(v_id ==  "merchant-pay-code") {  
							v_action = "viewpaymentcode";
						} 
