...
$(document).ready(function () {
	...
	$.each(json, function(i, v) {
		...
		var text="";
		var actiontype="";
			if( REQUEST_TYPE == 'New Limit') {
				text = "<div class='label label-success'  align='center' >"+REQUEST_TYPE+"</div>";
				actiontype="NEW_LIMIT";
			}else if( REQUEST_TYPE == 'Modify Limit') {
				text = "<div class='label label-info' align='center' >"+REQUEST_TYPE+"</div>";
				actiontype="MODIFY_LIMIT";
			}
			
		
		var appendTxt = "<tr class="+addclass+" id='"+rowindex+"' index='"+rowindex+"'  > "+
		"<td ><input class='center-chk' type='radio' id='radbutt' name='radbutt' value='"+PRD_CODE+","+actiontype+","+REF_NO+"' />    </td>"+
			"<td >"+PRD_CODE+"</span></td>"+
			"<td>"+PRD_NAME+"</span></td>"+
			"<td>"+text+"</span></td>"+
			/* "<td><p><a id='B"+REF_NO+"' class='btn btn-success' href='AuthLimitsettingsdataview.action?limitCode="+PRD_CODE+"&actiontype="+actiontype+"' index='"+rowindex+"'>Authorise</a></p></td>";
			 */"</tr>";
			i++;
...
$(function() {
	...
	$("#btn-submit").click(function(event) {
		...
		var searchIDs="";
 		...
 		$("input:radio:checked").each(function(index) {
 		    ...
 		    $('#refno').val(searchIDs.split(",")[2]);
 		 	...
 		 	$('#limitCode').val(searchIDs.split(",")[0]); 
		});
        ...
        if(searchIDs.length == 0) {
			...
			var url="${pageContext.request.contextPath}/<%=appName %>/AuthLimitsettingsdataview.action";
			$("#form1")[0].action=url;
			$("#form1").submit(); 
