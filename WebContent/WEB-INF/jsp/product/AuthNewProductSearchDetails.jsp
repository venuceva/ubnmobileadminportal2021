	...
	$.each(json, function(i, v) {
		...
		var actiontype="";
			if( REQUEST_TYPE == 'New Product') {
				...
				actiontype="NEW_PRODUCTS";
			}else if( REQUEST_TYPE == 'Modify Product') {
				text = "<div class='label label-info' align='center' >"+REQUEST_TYPE+"</div>";
				actiontype="MODIFY_PRODUCTS";
			}else if( REQUEST_TYPE == 'Active') {
				text = "<div class='label label-info' align='center' >"+REQUEST_TYPE+"</div>";
				actiontype="ACTIVE_PRODUCTS";
			}else if( REQUEST_TYPE == 'Deactive') {
				text = "<div class='label label-info' align='center' >"+REQUEST_TYPE+"</div>";
				actiontype="ACTIVE_PRODUCTS";
			}
		...
		var appendTxt = "<tr class="+addclass+" id='"+rowindex+"' index='"+rowindex+"'  > "+
			...
			"<td>"+PRD_NAME+"</span></td>"+
			"<td>"+text+"</span></td>"+
/* 			"<td><p><a id='B"+REF_NO+"' class='btn btn-success' href='AuthProductView.action?productCode="+PRD_CODE+"&actiontype="+actiontype+"&refno="+REF_NO+"' index='"+rowindex+"'>Authorise</a></p></td>";
 */			"</tr>";
			i++;
...
$(function() {
	...
	$("#btn-submit").click(function(event) {
		...
		var searchIDs="";
 		...
 		$("input:radio:checked").each(function(index) {
 		    ...
 		    $('#refno').val(searchIDs.split(",")[2]);
 		 	...
 		 	$('#productCode').val(searchIDs.split(",")[0]); 
		});
        ...
        if(searchIDs.length == 0) {
			...
			var url="${pageContext.request.contextPath}/<%=appName %>/AuthProductView.action";
			$("#form1")[0].action=url;
			$("#form1").submit(); 
		...
		<div id="content" class="span10">  
        	...
        	<div class="row-fluid sortable"><!--/span--> 
				<div class="box span12"> 
					<div class="box-header well" data-original-title>Product Approval Details
					  <div class="box-icon"> 
