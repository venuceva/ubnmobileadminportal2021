	...
	GraphTable.prototype.init = function(thing) {
		...
		if (!self.multipleDataSets) {
			...
			for (var i = 0, len = self.xLength; i < len; ++i) {
				...
				row += '<td>' + (i+1) + '</td><td>' + self.x[i] + '</td>' + tds + '</tr><tr>';
			}
		}
	...
	$.fn.graphify = $.fn.graphify || function(options) {
		...
		var buttons = (function() {
			...
			for (var i = 0, len = types.length; i < len; ++i) {
				var actn=types[i]+id+"()";
				btns += '<a class="btn btn-success" onclick='+actn+' id="' + id + '-graphify-button-' + types[i] + '" >' +
		...
		//click handlers
		//changing graph type
		$(document).on('click', 'a[id^="' + id + '-graphify-button-"]', function() {
